...
ft_ip = 'http://192.168.0.103:80' # yifan laptop
# ft_ip = 'http://192.168.0.103:8080' # mei laptop
class PegInHoleTask():
    ...
    def __init__(self, controller, T, start_position_file, dataset_dir, task_name, task_args,
                stop_event, stop_lock, pause_event, pause_lock, mode = None, task_timer = 60, 
                VFE_params =None, no_delta = True):
        print("Initializing PegInHole...")
        assert mode in ['legacy','collection']
        ...
        ## General settings
        global_gain = 0.6 # for writing #0.25 for wiping on dish # 0.75 in data collection, 0.5 in execution
        control_mode = "pid"
        ...
        if control_mode == "pid":
            ...
            if self.mode == 'collection':
                # randomlize initial kp 
                if self.current_task['args'].get('randomizing_control', False):
                    kp_Fz_log_range = self.current_task['args']['random_kp_Fz_logrange']
                    kp_Fz_2_speedcap_ratio_logrange = self.current_task['args']['kp_Fz_2_speedcap_ratio_logrange']
                    kp_Fx_log_range = self.current_task['args']['random_kp_Fx_logrange']
                    kp_Fx_2_speedcap_ratio_logrange = self.current_task['args']['kp_Fx_2_speedcap_ratio_logrange']
                    pid_P_Fz = -max(0.0008, np.exp(np.random.uniform(kp_Fz_log_range[0],kp_Fz_log_range[1]))) ##make sure not too small at the start
                    pid_P_Fx = -max(0.0005, np.exp(np.random.uniform(kp_Fx_log_range[0],kp_Fx_log_range[1])))
                    self.pid_P_Fz = pid_P_Fz
                    self.pid_P_Fx = pid_P_Fx
                    self.kp_Fz_2_speedcap_ratio = max(1, np.exp(np.random.uniform(kp_Fz_2_speedcap_ratio_logrange[0],\
                                                kp_Fz_2_speedcap_ratio_logrange[1]))) #make sure not too small at the start
                    self.kp_Fx_2_speedcap_ratio = np.exp(np.random.uniform(kp_Fx_2_speedcap_ratio_logrange[0],\
                                                kp_Fx_2_speedcap_ratio_logrange[1]))
                    pid_I = 0 #-0.005*pid_scale # during data collectoin
                    pid_D = 0#-0.01*pid_scale
                    self.pid_z = simple_pid.PID(pid_P_Fz, pid_I, pid_D, setpoint=None)
                    self.pid_z.sample_time = 0.1  # Update every 0.1 seconds
                    self.pid_y = simple_pid.PID(pid_P_Fx, pid_I, pid_D, setpoint=None)
                    self.pid_y.sample_time = 0.1  # Update every 0.1 seconds
                    self.pid_x = simple_pid.PID(pid_P_Fx, pid_I, pid_D, setpoint=None)
                    self.pid_x.sample_time = 0.1  # Update every 0.1 seconds
                else:
                    pid_P, self.pid_P = -0.003, -0.003
                    #estimator in the loop
                    #pid_P, self.pid_P = -0.001, -0.001
                    ...
                    self.pid_y.sample_time = 0.1  # Update every 0.1 seconds
                    self.pid_x = simple_pid.PID(pid_P, pid_I, pid_D, setpoint=None)
                    self.pid_x.sample_time = 0.1  # Update every 0.1 seconds
            elif self.mode == 'legacy':
                if task_name == 'insert_object_2D':
                    pid_P = -0.005 #-0.002*pid_scale
                    pid_I = 0 #-0.005*pid_scale # during data collectoin
                    pid_D = 0#-0.01*pid_scale
                    self.pid_z = simple_pid.PID(pid_P, pid_I, pid_D, setpoint=None)
                    self.pid_z.sample_time = 0.1  # Update every 0.1 seconds
                    ...
                    self.pid_x = simple_pid.PID(pid_P, pid_I, pid_D, setpoint=None)
                    self.pid_x.sample_time = 0.1  # Update every 0.1 seconds
                elif task_name == 'track_force_position_trajectory':
                    pid_P = -0.005 #-0.002*pid_scale
                    pid_I = 0 #-0.005*pid_scale # during data collectoin
                    pid_D = 0#-0.01*pid_scale
                    self.pid_z = simple_pid.PID(pid_P, pid_I, pid_D, setpoint=None)
        ...
        ##########################################
        ### Recording in separate thread instead #
        ##########################################
        if self.mode == 'legacy':
            # iterate until folder number is unseen, then create folder
            counter = 0
            record_test_data_dir = f"/home/grablab/VisionForceEstimator/real_world_data/experiment_data/experiment_{counter}/"
            while os.path.exists(record_test_data_dir):
                counter += 1
                record_test_data_dir = f"/home/grablab/VisionForceEstimator/real_world_data/experiment_data/experiment_{counter}/"
            os.makedirs(record_test_data_dir)

            # record images in the same folder as predictions and ground truth
            self.record_dataset_img_dir = os.path.join(record_test_data_dir, 'images/')
            os.makedirs(self.record_dataset_img_dir, exist_ok=True)

            # create file paths for saving the pickle files
            self.record_file = record_test_data_dir + 'vision_recorded_forces.pkl'
            self.record_file_ground_truth = record_test_data_dir + "vision_ground_truth.pkl"
            self.record_file_CF = "/data_experiments/forces/vels_FTs.pkl"
            self.recorded_delta_z = []

            print("Experiment data folders created at:" + record_test_data_dir)
        ...
        if mode == 'legacy':
            ...
            if 'VFE' in self.force_source:
                ...
                model_path = VFE_params['model_path'] #'/home/grablab/Downloads/1231_h5_int2_hz10.pt'
                # model_path = '/home/grablab/Downloads/best_val.pt'
                n_history = VFE_params['n_history'] #5
                history_interval = VFE_params['history_interval']
                ...
                self.vfe = VFEstimator(nn=nn.network, cam=self.cam, n_history=n_history, history_interval=history_interval,\
                                       SAM = sam_model, Hz = 10)
        ...
        self.rate = 10 #40 during data collection, 20 during testing
        self.t = 0
        self.last_contact_time = math.inf
        self.move = True
        # self.move = False
        ...
        while True:
            ...
            if self.mode=='legacy':
                self.read_forces() # TODO: uncomment to filter VFE forces for control
                # self.read_forces(is_filter=False)
            ...
            if self.current_task['args'].get('randomizing_control', False):
                speed_cap = math.fabs(self.pid_P_Fx)*self.kp_Fx_2_speedcap_ratio
                speed_cap_z = math.fabs(self.pid_P_Fz)*self.kp_Fz_2_speedcap_ratio
                print('Current speed cap xy is:', speed_cap)
                print('Current speed cap z is:', speed_cap_z)
            else:
                if task_name  == 'insert_object_2D':
                    speed_cap = 0.004
                    speed_cap_z = 0.002
                    ...
                    speed_cap = 0.004 # 0.01*global_gain
                    speed_cap_z = 0.004 #0.0002 #0.01*global_gain
                    ## estimator in the loop
                    # speed_cap = 0.001 # 0.01*global_gain
                    # speed_cap_z = 0.002 #0.0002 #0.01*global_gain
            speed_cap_rot = 0.001*0.4
            speed_cap_rot_z = 0.06 #0.01*2

            if self.current_task["name"] ==  'track_force_position_trajectory':
                gains = None
            else:
                gains = [speed_cap, speed_cap, speed_cap_z, speed_cap_rot, speed_cap_rot, speed_cap_rot_z]
            target_motion_vels = [0, 0, 0, 0, 0, 0]

            ## Specifically for the task of track_force_position_trajectory
            if self.current_task["name"] ==  'track_force_position_trajectory':
                if not 'forcePositionTrajectory' in self.task_vars.keys():
                    from force_position_trajectory import forcePositionTrajectory
                    self.task_vars.update({"forcePositionTrajectory":\
                        forcePositionTrajectory(task_args['forces'], task_args['positions'], speed_cap, speed_cap_rot_z, \
                                    xy_offset = self.controller.get_EE_transform()[1][0:2])})
                    ic(self.task_vars['forcePositionTrajectory'].total_time())
            ...
            # ic(target_motion_vels)
            if target_motion_vels == 'end':
                print('task ended')
                break
            ...
            # Scaling the velocities to ensure no component is higher than its limit (if speed_capso: the entire vector is reduced iteratively until all limits are enforced)
            ...
            print("Force Readings for :")
            print(self.force_source)
            #pprint(["{:.2f}".format(x) for x in self.delta_from_floating])
            print("Y :", "{:.2f}".format(self.delta_from_floating[1]), " - ", "Z :", "{:.2f}".format(self.delta_from_floating[2]))
            if 'VFE' in self.force_source:
                print("Error forces:")
                error_thr = 0.2
                ...
                print("Y :", "{:.2f}".format(self.force_data_GT[1]-self.force_data[1]), " - ", "Z :", "{:.2f}".format(self.force_data_GT[2]-self.force_data[2]))
    ...
    def get_target_motion(self, task_type="still", args=None, gains=[0.001, 0.001, 0.001, 0, 0, 0]):
        ...
        if task_type == "still":
        ...
        elif task_type == "orient_towards":
            # print("Orienting")
            vels = self.orient_towards()
        elif task_type == "slide":
            vels = self.slide(axis=args["slide_axis"])
        elif task_type == "slide_and_orient":
            if abs(self.force_data[0]-self.average_x) > 1 or abs(self.force_data[1]-self.average_y) > 1:
                ...
                if args["slide_axis"] == "-y":
                    vels = [0, -1, 0, 0, 0, 0]
        elif task_type == "slide_and_climb":
            vels = self.explore_laterally(axis=args["slide_axis"])
        elif task_type == "test_contact":
            vels = self.test_contact(frozen_axis=args["frozen_axis"], initial_contact_target=args["initial_contact_target"])
        elif task_type == "align_with_plane_below":
            vels = self.align_with_plane_below()
        elif task_type == "slide_and_test":
            vels = self.slide_and_test(args["slide_axis"])
        elif task_type == "direct_control":
            vels = self.direct_control()
        elif task_type == "explore_hole":
            vels = self.explore_hole(args["start_direction"])
        elif task_type == "rotate_against_edge":
            vels = self.rotate_against_edge()
        elif task_type == "insert_object":
            vels = self.insert_object()
        elif task_type == "insert_object_2D":
            vels = self.insert_object_2D()
        elif task_type =="explore_hole_andy":
            vels = self.explore_hole_andy()
        elif task_type == "wipe":
            vels = self.wipe(args["is_randomizing_force"])
        elif task_type == "touch_walls":
            vels = self.touch_walls(args["is_randomizing_force"], args["mode"], args["is_stick_to_wall"])
        elif task_type == "reposition_fingers":
            vels = self.reposition_fingers(args["target_location"], args["target_orientation"])
        elif task_type == "random_move":
            vels = self.random_move()
        elif task_type == "track_force_position_trajectory":
            vels = self.track_force_position_trajectory()
            return vels
        elif task_type == 'measure':
            self.current_contact_target = [None, None, None]
            return [0,0,0,0,0,0]
        else:
            raise Exception('Unsupported task_type: ' + task_type)

        return self.apply_gains(vels, gains)

    def track_force_position_trajectory(self, R_EE_in_World =[0,0,-1, 
                math.sqrt(2)/2,math.sqrt(2)/2,0,
                math.sqrt(2)/2,-math.sqrt(2)/2,0], EE_heading = math.pi/2):
        """
        Args:
        - force_position_trajectory: (force_trajectory, position_trajectory) both are Klampt Trajectory
        ...
        if self.task_vars["task"] == "down":
            ...
            force_target_0, _, _ = self.task_vars['forcePositionTrajectory'].get_force_position_heading(0)
            self.current_contact_target = [None,None,-force_target_0[0]]
            self.maintain_override = True
            # Transition from down to slide
            if self.last_contact_time < 0.1:
                self.task_vars["task"] = "track_force_position"
                self.task_vars.update({"start_time": time.time()})
        elif self.task_vars["task"] == "track_force_position":
            self.maintain_override = False
            current_time = time.time() - self.task_vars['start_time']
            if current_time > self.task_vars['forcePositionTrajectory'].total_time() + 1:
                return 'end'
            force, position, heading = self.task_vars['forcePositionTrajectory'].get_force_position_heading(current_time)
            current_R_EE, current_t = self.controller.get_EE_transform()
            ## Transform from EE to gripper in world
            # current_R = so3.mul(current_R_EE, so3.inv(R_EE_in_World)) ## gripper R
            # current_rotation_vector = so3.moment(current_R)
            # current_R = so3.from_moment([0,0,current_rotation_vector[2]])  #keep only z rotation

            # target_R_EE = so3.mul(so3.from_moment([0,0,heading[0]]),R_EE_in_World) #no idea how this worked??
            target_R_EE = so3.mul(so3.from_moment([0,0,heading[0] - EE_heading]), R_EE_in_World) #rotate w.r.t. home EE pose
            # ic(heading[0], EE_heading)
            z_angle_difference = so3.error(target_R_EE, current_R_EE) #not substracting heading directly because of wrapping-up issue
            # ic(target_R_EE, current_R_EE, z_angle_difference)
            # ic(z_angle_difference, position, current_t)
            xy_vels = list((np.array(position) - np.array(current_t[0:2]))/dt)
            vels = xy_vels + [0,0,0, z_angle_difference[2]/dt]
            for i in range(len(vels)):
                try:
                    vels[i] = vels[i].item()
                except:
                    pass
            self.current_contact_target = [None, None, -force[0]]
            try:
                self.current_contact_target[2] = self.current_contact_target[2].item()
            except:
                pass
        # ic(vels)
        # ic(self.current_contact_target)
        return vels
    ...
    def randomize_control(self):
        ...
        kp_Fx_log_range = self.current_task['args']['random_kp_Fx_logrange']
        kp_Fx_2_speedcap_ratio_logrange = self.current_task['args']['kp_Fx_2_speedcap_ratio_logrange']
        pid_P_Fz = -np.exp(np.random.uniform(kp_Fz_log_range[0],kp_Fz_log_range[1])) 
        pid_P_Fx = -np.exp(np.random.uniform(kp_Fx_log_range[0],kp_Fx_log_range[1]))
        self.pid_P_Fz = pid_P_Fz
        self.pid_P_Fx = pid_P_Fx
        self.kp_Fz_2_speedcap_ratio = np.exp(np.random.uniform(kp_Fz_2_speedcap_ratio_logrange[0],\
                                    kp_Fz_2_speedcap_ratio_logrange[1]))
        self.kp_Fx_2_speedcap_ratio = np.exp(np.random.uniform(kp_Fx_2_speedcap_ratio_logrange[0],\
                                    kp_Fx_2_speedcap_ratio_logrange[1]))

        self.pid_z = simple_pid.PID(self.pid_P_Fz, 0, 0, setpoint=None)
        self.pid_x = simple_pid.PID(self.pid_P_Fx, 0, 0, setpoint=None)
    ...
    def wipe(self, is_randomizing_force=False):
        ...
        if self.current_task["args"]['randomizing_vel']:
            random_vel_range = self.current_task["args"]['random_vel_range']
        ...
        if "is_started" not in self.task_vars:
            ...
            workspace_size = self.current_task["args"].get('workspace_size', 0.06) #0.1 before 0115 # m
        ...
        if self.task_vars["task"] == "down":
            ...
            # Transition from down to slide
            if self.last_contact_time < 0.1:
                # randomize starting slide direction during data collection
                if self.mode == 'collection':
                ...
                # randomize wiping velocity
                if self.current_task["args"]['randomizing_vel']:
                    self.vel_magnitude = np.random.uniform(random_vel_range[0], random_vel_range[1])
                else:
                    self.vel_magnitude = 1 #0.5

                if self.current_task["args"]['randomizing_control']:
                    self.randomize_control()
            ...
            # self.current_contact_target = [None, None, None]
            self.maintain_override = False
            vels = [0., self.vel_magnitude, 0., 0., 0., 0.]
            ...
            # debug 
            # if self.mode=='legacy':
            #     self.current_contact_target = [None, None, -1]
        ...
        if is_randomizing_force:
            if self.task_vars["task"] != "down": ## do not randomize contact target when seeking contact 
    ...
    def insert_object(self):
        ...
        if self.task_vars["state"] == "down":
            self.current_contact_target = [None, None, -downward_strength]
    ...
    def insert_object_2D(self):
        ...
        insertion_done = False
        (t, R) = self.get_pos()
        lateral_strength = 0.3 #0.5
        downward_strength = 0.3
        ...
        if self.task_vars["state"] == "down":
            self.current_contact_target = self.standard_gravity_target
            if self.last_contact_time < 0.01: #downward_strength: # Regulation finished, switch to slide1
                self.last_contact_time = math.inf
                self.task_vars.update({"state":"slide1"})
            else:
                vels = [0, 0, 0, 0, 0, 0] # Do nothing, let the regulation work
        ...
        if self.task_vars["state"] == "slide1":
            ...
            if 'contact_target_for_insertion' in self.task_vars.keys():
                ...
                self.current_contact_target = (None, lateral_strength, -downward_strength)
                if self.last_contact_time < 0.01: # Regulation finished, switch to slide1
                    self.last_contact_time = math.inf
                    self.task_vars.update({"state":"insert"})
                else:
                    vels = [0, 0, 0, 0, 0, 0] # Do nothing, let the regulation work
        ...
        if self.task_vars["state"] == "insert":
            ...
            self.current_contact_target = [None, 0, self.standard_gravity_target[2]] #use this for the normal stuff
            vels = [0, 0, 0, 0, 0, 0] # Do nothing, let the regulation work
    ...
    def maintain_side_contact_pid(self, target_forces=None, gain = 1):
        ...
        vels = [None, None, None]
        if target_forces[0] is not None:
            print("Regulating on X...")
            self.pid_x.setpoint = target_forces[0]
            vels[0] = self.pid_x(self.delta_from_floating[0])
            # vels[0] = self.pid_x(self.force_data_GT[0])
        if target_forces[1] is not None:
            print("Regulating on Y...")
            self.pid_y.setpoint = target_forces[1]
            vels[1] = -self.pid_y(self.delta_from_floating[1])
            # vels[1] = -self.pid_y(self.force_data_GT[1])
        if target_forces[2] is not None:
            print("Regulating on Z...")
            self.pid_z.setpoint = target_forces[2]
            if self.current_task['args'].get('regulate_Fz_with_total_force', False):
                #vels[2] = self.pid_z(np.linalg.norm(self.delta_from_floating[2]) ## need to be debugged 
                pass
            else:
                vels[2] = self.pid_z(self.delta_from_floating[2])
            # vels[2] = self.pid_z(self.force_data_GT[2])
    ...
    def read_forces(self, is_filter=True, filter_number=50):
        '''
        Read forces from specified resources and updates parameters. 'ATI', 'franka', or 'VFE'

        Arg(s):
            is_filter : boolean
                Whether or not the forces go through a low pass filter (i.e. gets averaged) during online predictions. If
                filtering occurs, the forces values are averaged with neighboring values based
                on history size. Defaults to True.
            filter_number : int
                Number of samples to use to smooth the VFE predictions.
        '''
        history_size = 1 #10 # For computing the smoothed forces
        ...
        if 'franka' in self.force_source:
            ...
            self.force_data_GT = copy.deepcopy(self.force_data)
        if 'ATI' in self.force_source:
        ...
        if self.mode == 'legacy' and self.record_forces:
            raw_force_data = copy.deepcopy(self.force_data)
            # filter forces based on history size
            if is_filter:
                # Smoothing the force readings
                if len(self.last_forces) < filter_number:
                ...
                for force in self.last_forces[-history_size:]:
                    for i in range(len(averages)):
                        averages[i] += force[i]
                for i in range(len(averages)):
                    averages[i] /= history_size
                # save raw force data for recording, use filtered data for online control
                ...
                for i in range(len(self.force_data)):
                    self.delta_from_floating[i] = self.force_data[i] # TODO: uncomment to use VFE predictions
                    # self.delta_from_floating[i] = self.force_data_GT[i]

            # record at designated frequency
            if time.time() - self.record_timer > 1 / self.record_frequency:
                self.record_timer = time.time()
                self.recorded_forces.append(raw_force_data)
                if self.force_data_GT is not None:
                    self.recorded_forces_ground_truth.append(self.force_data_GT)
                # record ground truth and predicted forces
                ...
                # record images
                color_image, _ = self.cam.get_rgb_frames()
    ...
    def get_vels(self):
        vels = self.controller.get_EE_velocity(tool_center = se3.identity())
        return ( (vels[0], vels[1], vels[2]), (vels[3], vels[4], vels[5]))

    def get_pos(self):
        (R, t) = self.controller.get_EE_transform(tool_center = se3.identity())
        return (t, R)
